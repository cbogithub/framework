...
public class Movii_Inactive_Session_Test extends Movii_Core_Page
{
	Movii_Login_Page login_obj = new Movii_Login_Page();

	@BeforeClass(alwaysRun = true)
	public void isSkip() throws MalformedURLException, IOException {
		if (!TestUtil.isExecutable("Movii_Inactive_Session_Test")) {
			throw new SkipException("Skipping the test as the Run mode is No");
		} else {
			movi();
			Movii_Core_Page.logger = Movii_Core_Page.report.createTest("Movii Inactive Session Test", "Test Case for User Inactive session ");
		}
	}
