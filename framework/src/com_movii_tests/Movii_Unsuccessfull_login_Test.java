...
public class Movii_Unsuccessfull_login_Test extends Movii_Core_Page
{
	Movii_Login_Page login_obj = new Movii_Login_Page();

	@BeforeClass(alwaysRun = true)
	public void isSkip() throws MalformedURLException, IOException {
		if (!TestUtil.isExecutable("Movii_Unsuccessfull_login_Test")) {
			throw new SkipException("Skipping the test as the Run mode is No");
		}
		else {
			movi();
			Movii_Core_Page.logger = Movii_Core_Page.report.createTest("Movii_Unsuccessfull_login_Test", "Test Case for Unsuccessfull Login");
		}
	}

	@Test(priority = 1)
	public void unsuccessfull_login() throws IOException {
		login_obj.user_unsuccessfull_login();
	}
